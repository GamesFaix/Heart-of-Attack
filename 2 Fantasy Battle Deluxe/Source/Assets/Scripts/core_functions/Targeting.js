...
//target cell - arc
/*function T333(unit: GameObject, rng: int, startcell: GameObject): boolean{//arc attack
	targetcell=null;
	var distance=1;
	var arrayflip=true;
	var cells1=new Array();
	var cells2=new Array();
	cells1.push(startcell);
	while(distance<=rng){
		if (arrayflip==true) {list1=cells1; list2=cells2;}
		else {list1=cells2; list2=cells1;}
		while(list1.length>0){
			var cell=list1[0];
			check=0;
			while (check<celllist.length){
				if (IsAdjacent(cell,celllist[check])==true
				&& IsLegal(celllist[check])==false){
					if(HasImpassible(celllist[check])==false){
						MakeLegal(celllist[check]);
					}
					if(Vacant(celllist[check])==true){
						list2.push(celllist[check]);
					}
				}		
				check+=1;	
			}
	...
	check=0;
	while (check<celllist.length){
		if (celllist[check].GetComponent(CellProperties).legal==true){
			cells1.push(celllist[check]);
		}	
		check++;
	}
	check=0;
	while (check<cells1.length){
		if(ArcExclusion(unit, startcell,cells1[check])==false){
			cells1[check].GetComponent(CellProperties).legal=false;
		}
		check++;
	}
...
/*function ArcExclusion(unit: GameObject, startcell: GameObject, othercell: GameObject): boolean{
	//Debug.Log("arc exclusion");
	var cell1=startcell.GetComponent(CellProperties);
	var cell2=othercell.GetComponent(CellProperties);
	if ((cell1.gameX==0 && cell2.gameX==0)||(cell1.gameX==count && cell2.gameX==count)
	||(cell1.gameY==0 && cell2.gameY==0)||(cell1.gameY==count && cell2.gameY==count)
	||(cell1.gameZ==0 && cell2.gameZ==0)||(cell1.gameZ==count && cell2.gameZ==count)){
		var hits: RaycastHit[];
		hits=Physics.RaycastAll(startcell.transform.position,othercell.transform.position,Mathf.Infinity);
		
		if (hits.length>0){Debug.Log("hit 0:"+hits[0].transform);}
		if (hits.length>1){Debug.Log("hit 1:"+hits[1].transform);}
		if (hits.length>2){Debug.Log("hit 2:"+hits[2].transform);}
		if (hits.length>3){Debug.Log("hit 3:"+hits[3].transform);}
		if (hits.length>4){Debug.Log("hit 4:"+hits[4].transform);}
		
		var check=0;
		while (check<hits.length){
			var thisRay= new Ray(startcell.transform.position, othercell.transform.position);
			drawRays[rayCount]=thisRay;
			rayCount++;
			var hitTransform=hits[check].transform;
			var hitObject=hitTransform.gameObject;
			
			Debug.Log(hitObject);
			if (hitObject.GetComponent(ObjectStats) && hitObject!=unit){
...
function IsLinear(startcell:GameObject,othercell:GameObject): boolean{//(unused)
	var cellA: CellProperties = startcell.GetComponent(CellProperties);
	var cellB: CellProperties = othercell.GetComponent(CellProperties);
