...
function Awake(){//retrieve other components
	...
	sideBar=Resources.Load("gui/panel_background") as Texture2D;
	camMain=Camera.main.GetComponent(CamMain);
}	
function Start(){//yield to LoadIcons
	yield LoadIcons();
}
function LoadIcons():IEnumerator{//retrieve icons
	//icons
...
function SidePanel(){//displays side info panel

	GUI.DrawTexture(Rect(0,0,180,Screen.height),sideBar,ScaleMode.StretchToFill,true,0);
	
	if (viewedobject){
		var viewedstats: ObjectStats = viewedobject.GetComponent(ObjectStats);
		
		if (viewedstats.owner){//draw sideBar to match viewed unit ownership
			GUI.DrawTexture(Rect(5,5,170,Screen.height-10),gameindex.colors[gameindex.playerColors[viewedstats.owner]],ScaleMode.StretchToFill,true,0);
		}
		//title & thumbnail
		if(GUI.Button(Rect(10,10,160,20),viewedstats.objname,gui_styles.sButton[gameindex.playerTeams[viewedstats.owner]])){camMain.CamFocus(viewedobject);}
...
function ActionInfo(object: GameObject, act1to9: byte){//displays action info
	...
	//var func: String = stats.actText[act1to9,eFunc];
	
	var ap: byte = stats.actNums[act1to9,eAp];
	var fp: byte = stats.actNums[act1to9,eFp];
	var used: boolean = actionCoord.actUsed[act1to9];
...
function DisplayWatchedUnits(){
	...
	for (i=0; i<3; i++){
		if (watched[i]){
			...
			if (stats.owner){
				GUI.DrawTexture(Rect(xPos+3+(height*i),yPos+3,height-6,height-6),gameindex.colors[gameindex.playerColors[stats.owner]],ScaleMode.StretchToFill,true,0);
			}
			if (GUI.Button(Rect(xPos+2+(height*i),yPos+2,height-4,height-4),stats.thumb)){
				if (Input.GetKey("left shift")) {watched[i]=null;}
				else {
					View(watched[i]);
					camMain.CamFocus(watched[i]);
				}
			}
		}	
	}
...
*/
		...
		//
//user input
function MouseOnGUI(): boolean{//returns true if mouse is over GUI, false if mouse over map
	...
	if (e.isKey && e.type.KeyUp){
		if(showMenu==false){
			...
			if (msgEnable==false){
				...
				if (e.character=="e"){camMain.CamFocus(currentunit);}
				if (e.character=="d"){View(currentunit);}
				if (e.character=="r"){camMain.CamFocus(viewedobject);}
...
function QueueDisplay(){//draws currentunit button
	...
	if (queue.queuelist){
		...
		GUI.DrawTexture(Rect(xPos+3,yPos+23,width-6,24),gameindex.colors[gameindex.playerColors[currentunitstats.owner]],ScaleMode.StretchToFill,true,0);
		...
		if (showQ==true){
			...
			for (i=0; i<qIndicies; i++){
				if (queue.queuelist.Count>i){
					...
					GUI.DrawTexture(Rect(xPos+2,yPos+70+(qHeight*i)+2,width-4,qHeight-4),gameindex.colors[gameindex.playerColors[unitstats.owner]],ScaleMode.StretchToFill,true,0);
					if (GUI.Button(Rect(xPos,yPos+70+(qHeight*i),width,qHeight),unitstats.objname)){
						View(unit);
						camMain.CamFocus(unit);
					}
				}
			}
		}
...
function GameMenu(){
	...
	//background box
	if (showMenu==true){
		GUI.DrawTexture(Rect(xPos+3,yPos+3,width-6,height-6),sideBar,ScaleMode.StretchToFill,true,0);
		...
		if (showControls==false){
			...
			if(GUI.Button(Rect(xPos+10,yPos+10+btnHeight,width-20,btnHeight),"Fullscreen On/Off")){Screen.fullScreen=!Screen.fullScreen;}
			if(GUI.Button(Rect(xPos+10,yPos+10+btnHeight*2,width-20,btnHeight),"Controls")){showControls=true;}	
			if(GUI.Button(Rect(xPos+10,yPos+10+btnHeight*3,width-20,btnHeight),"Help")){
				gui_master.view="help";
				gui_help.previousview="game";
			}		
			if(GUI.Button(Rect(xPos+10,yPos+10+btnHeight*4,width-20,btnHeight),"Return to Main Menu")){Application.LoadLevel(0);}
			if(GUI.Button(Rect(xPos+10,yPos+10+btnHeight*5,width-20,btnHeight),"Exit Program")){Application.Quit();}
		}
