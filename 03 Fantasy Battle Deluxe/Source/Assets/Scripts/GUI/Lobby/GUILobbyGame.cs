...
public class GUILobbyGame : MonoBehaviour {
	...
	void FactionSelector (Panel p, Player player) {
		...
		if (selectee != default(Player)) {
			for (int i=0; i<4; i++){
				if (i < Roster.FreeFactions().Count){
					if (GUI.Button(p.Box(0.25f), Roster.FreeFactionNames()[i])) {
						InputBuffer.Submit(new RRosterAssign(Source.ActivePlayer(), selectee, Roster.FreeFactions()[i]));
